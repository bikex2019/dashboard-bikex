...
<div class="centres col-md-12 mt-5" style="margin: 0 auto">
        ...
        <form> 
            ...
            <!-- <input type="text" class="form-control" placeholder="dearch vehicle" v-model="search_vehicle">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                  <td  v-on:click="addrow()" style="cursor:pointer;background-color:#ffb52f">
                  <i class="fa fa-plus pr-3" aria-hidden="true" ></i>Add Row
                  </td>
                </tr>
                <tr style="border:none">
                    <td colspan="3" class="m-0 p-0 text-right pt-3 pr-3"> <strong>Total:</strong></td>
                    <td class="m-0  text-left">Rs.{{itemPrice}} </td>
                    <td class="m-0  text-left">Rs.{{total}}</td>
                    <td colspan="2" class="m-0  text-left">Rs.{{grandTotal}}</td>
                </tr>
            </tbody>
        </table>
    </div>
      <div class="col-md-12 mb-4">
          ...
          <button v-if="hasValue" type=" bt" class="custom2 px-5" v-on:click="updaterefurb">Update</button>  
          <button v-else type=" bt" class="custom2 px-5" v-on:click="addrefurb">Add Details</button>  
...
export default {
    ...
    created(){
      this.loading = true
    this.refurb_id = this.$route.query.id
  window.console.log(this.id)
    this.$http.get('https://backend-bikex.herokuapp.com/api/refurbished/'+ this.refurb_id)
          .then(response=>{
          this.refurbish = response.body
          window.console.log(this.refurbish[0])
          if(this.refurbish.length == 0){
            this.hasValue = false
          }else{
          this.hasValue = true
          this.input = this.refurbish[0].parts_changed
          this.comments = this.refurbish[0].comments
          this.loading = false
          }
         }).catch(()=>{
           this.loading = false
    ...
    methods:{
       updaterefurb: function(){
         window.console.log(this.id)
            this.loading = true
            this.$http.put('https://backend-bikex.herokuapp.com/api/refurbished/'+ this.refurbish[0]._id,{
            vehicle_number: this.refurb_id,
        ...
        addrefurb(){
           this.loading = true
            ...
            then(()=>{
            this.loading= false
            this.$swal('Refurbishment details has been Added.');
            window.location.reload()
            }).catch(()=>{
              this.$swal('Something is wrong!');
                this.loading =  false
